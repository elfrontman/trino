<?php
namespace Optimize\Modules\Shortcodes\Icon;

use Optimize\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package Optimize\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            ...
            'params'                    => array_merge(
                \OptimizeMikadoIconCollections::get_instance()->getVCParamsArray(),
                array(
                    array(
                        'type'        => 'dropdown',
                        'heading'     => 'Size',
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'optimize'),
    ...
    public function render($atts, $content = null) {
        ...
        $html = optimize_mikado_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.optimize_mikado_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.optimize_mikado_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.optimize_mikado_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.optimize_mikado_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.optimize_mikado_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.optimize_mikado_filter_px($params['border_radius']).'px';
                }
            }
        }
