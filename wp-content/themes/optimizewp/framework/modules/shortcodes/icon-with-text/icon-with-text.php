<?php
namespace Optimize\Modules\Shortcodes\IconWithText;

use Optimize\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class IconWithText
 * @package Optimize\Modules\Shortcodes\IconWithText
 */
class IconWithText implements ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(array(
			...
			'params'                    => array_merge(
				optimize_mikado_icon_collections()->getVCParamsArray(array(), '', true),
				array(
					array(
						'type'       => 'attach_image',
						'heading'    => 'Custom Icon',
					...
					array(
						'type'       => 'dropdown',
						'heading'    => 'Enable Icon Hover Animation?',
						'param_name' => 'enable_icon_animation',
						'value'      => array(
							'Default'  => '',
							'Enabled'  => true,
							'Disabled' => false
					...
					array(
						...
						'value'       => array_flip(optimize_mikado_get_text_transform_array(true)),
						'save_always' => true,
						'group'       => 'Text Settings'
					),
					array(
						...
						'value'       => array_flip(optimize_mikado_get_font_weight_array(true)),
						'save_always' => true,
						'group'       => 'Text Settings'
					),
					array(
	...
	public function render($atts, $content = null) {
		...
		$default_atts = array_merge($default_atts, optimize_mikado_icon_collections()->getShortcodeParams());
		...
		$params['holder_data'] = array();

		if($params['enable_icon_animation']) {
			$params['holder_data']['data-animation'] = true;
		}

		return optimize_mikado_get_shortcode_module_template_part('templates/iwt', 'icon-with-text', $params['icon_position'], $params);
	}
	...
	private function getIconParameters($params) {
		...
		if(empty($params['custom_icon'])) {
			$iconPackName = optimize_mikado_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
	...
	private function getHolderClasses($params) {
		...
		if(!empty($params['icon_position'])) {
			switch($params['icon_position']) {
				case 'top':
					$classes[] = 'mkdf-iwt-icon-top';
					break;
				case 'left':
					$classes[] = 'mkdf-iwt-icon-left';
					break;
				case 'right':
					$classes[] = 'mkdf-iwt-icon-right';
					break;
				case 'left-from-title':
					$classes[] = 'mkdf-iwt-left-from-title';
					break;
				default:
					break;
			}
	...
	private function getContentStyles($params) {
		...
		if($params['icon_position'] == 'left' && !empty($params['text_left_padding'])) {
			$styles[] = 'padding-left: '.optimize_mikado_filter_px($params['text_left_padding']).'px';
		}

		if($params['icon_position'] == 'right' && !empty($params['text_right_padding'])) {
			$styles[] = 'padding-right: '.optimize_mikado_filter_px($params['text_right_padding']).'px';
		}
