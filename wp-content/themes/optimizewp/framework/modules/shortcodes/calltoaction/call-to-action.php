<?php
namespace Optimize\Modules\CallToAction;
...
class CallToAction implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		$call_to_action_button_IconCollections = optimize_mikado_icon_collections()->iconCollections;
		...
		vc_map(array(
			...
			'params'                    => array_merge(
				...
				optimize_mikado_icon_collections()->getVCParamsArray(array(
					'element' => 'type',
				...
				array(
					...
					array(
						...
						'value'       => array_flip(optimize_mikado_get_btn_types(true)),
						'save_always' => true,
						'admin_label' => true,
					...
					array(
						...
						'value'       => array_flip(optimize_mikado_get_btn_hover_animation_types(true)),
						'save_always' => true,
						'admin_label' => true,
						'dependency'  => array('element' => 'show_button', 'value' => array('yes')),
						'group'       => 'Design Options',
					...
					array(
						...
						'value'       => array_merge(array('No Icon' => ''), optimize_mikado_icon_collections()->getIconCollectionsVC()),
						'save_always' => true,
						'dependency'  => array('element' => 'show_button', 'value' => array('yes'))
					)
				),
	...
	public function render($atts, $content = null) {
		...
		foreach(optimize_mikado_icon_collections()->iconCollections as $collection_key => $collection) {

			$call_to_action_icons_form_fields['button_'.$collection->param] = '';

		}

		$args = array_merge($args, optimize_mikado_icon_collections()->getShortcodeParams(), $call_to_action_icons_form_fields);
		...
		$html = optimize_mikado_get_shortcode_module_template_part('templates/call-to-action-template', 'calltoaction', '', $params);
	...
	private function getCallToActionStyles($params) {
		...
		if($params['box_border_color'] != '') {
			...
			if($params['box_border_width'] !== '') {
				$border_width = optimize_mikado_filter_px($params['box_border_width']);
			}
	...
	private function getCallToActionIcon($params) {
		...
		$icon                                   = optimize_mikado_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
		$iconStyles                             = array();
		$iconStyles['icon_attributes']['style'] = $this->getIconStyles($params);
		$call_to_action_icon                    = '';
		if(!empty($params[$icon])) {
			$call_to_action_icon = optimize_mikado_icon_collections()->renderIcon($params[$icon], $params['icon_pack'], $iconStyles);
		}
	...
	private function getButtonParameters($params) {
		...
		if(!empty($params['button_size'])) {
			$button_params_array['type'] = $params['button_size'];
		}
		...
		if(!empty($params['button_icon_pack'])) {
			...
			$iconPackName                       = optimize_mikado_icon_collections()->getIconCollectionParamNameByKey($params['button_icon_pack']);
