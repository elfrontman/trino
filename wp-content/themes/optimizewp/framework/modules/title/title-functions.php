...
if(!function_exists('optimize_mikado_get_title')) {
	/**
	 * Loads title area HTML
	 */
	function optimize_mikado_get_title() {
		$id = optimize_mikado_get_page_id();

		extract(optimize_mikado_title_area_height());
		extract(optimize_mikado_title_area_background());

		//check if title area is visible on page first, then in the options
		if(($meta_temp = get_post_meta($id, "mkdf_show_title_area_meta", true)) !== "") {
			$show_title_area = $meta_temp == 'yes' ? true : false;
		} else {
			$show_title_area = optimize_mikado_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
		}
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_type_meta", true)) !== "") {
			...
			$type = optimize_mikado_options()->getOptionValue('title_area_type');
		}
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_enable_breadcrumbs_meta", true)) !== "") {
			...
			$enable_breadcrumbs = optimize_mikado_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
		}

		//check if title color is set on page
		$title_color = '';
		if(($meta_temp = get_post_meta($id, "mkdf_title_text_color_meta", true)) !== "") {
			$title_color = 'color:'.$meta_temp.';';
		}

		//check if subtitle color is set on page
		$subtitle_color = '';
		if(($meta_temp = get_post_meta($id, "mkdf_subtitle_color_meta", true)) !== "") {
		...
		$parameters = apply_filters('optimize_mikado_title_area_height_params', $parameters);

		optimize_mikado_get_module_template_part('templates/title', 'title', '', $parameters);
	}
}

if(!function_exists('optimize_mikado_get_title_text')) {
	/**
	 * Function that returns current page title text. Defines optimize_mikado_title_text filter
	 ...
	 */
	function optimize_mikado_get_title_text() {
		...
		if(is_tag()) {
			...
			$title = single_term_title("", false).esc_html__(' Tag', 'optimize');
		...
		elseif(is_author()) {
			...
			$title = esc_html__('Author:', 'optimize')." ".get_the_author();
		...
		elseif(is_search()) {
			...
			$title = esc_html__('Search', 'optimize');
		} //is current page 404?
		elseif(is_404()) {
			//is 404 title text set in theme options?
			if(optimize_mikado_options()->getOptionValue('404_title') != "") {
				//get it from options
				$title = optimize_mikado_options()->getOptionValue('404_title');
			} else {
				//get default 404 page title
				$title = esc_html__('404 - Page not found', 'optimize');
			}
		} //is WooCommerce installed and is shop or single product page?
		elseif(optimize_mikado_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
		...
		elseif(optimize_mikado_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
			global $wp_query;
		...
		elseif(is_archive()) {
			$title = esc_html__('Archive', 'optimize');
		} //current page is regular page
		else {
			$title = get_the_title($id);
		}

		$title = apply_filters('optimize_mikado_title_text', $title);

		return $title;
	}
}

if(!function_exists('optimize_mikado_title_text')) {
	/**
	 * Function that echoes title text.
	 *
	 * @see optimize_mikado_get_title_text()
	 */
	function optimize_mikado_title_text() {
		echo optimize_mikado_get_title_text();
	}
}

if(!function_exists('optimize_mikado_subtitle_text')) {
	/**
	 * Function that echoes subtitle text.
	 *
	 */
	function optimize_mikado_subtitle_text() {
		$id       = optimize_mikado_get_page_id();
		$subtitle = '';
...
if(!function_exists('optimize_mikado_custom_breadcrumbs')) {
	/**
	 ...
	 * @see is_front_page()
	 * @see is_category()
	 * @see optimize_mikado_is_product_category()
	 * @see get_search_query()
	 */
	function optimize_mikado_custom_breadcrumbs() {
		global $post, $wp_query;

		$output      = "";
		$homeLink    = esc_url(home_url('/'));
		$pageid      = optimize_mikado_get_page_id();
		$bread_style = "";
		...
		$delimiter   = '<span class="mkdf-delimiter" '.optimize_mikado_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
		$home        = get_bloginfo('name'); // text for the 'Home' link
		$showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
		$before      = '<span class="mkdf-current" '.optimize_mikado_get_inline_style($bread_style).'>'; // tag before the current crumb
		$after       = '</span>'; // tag after the current crumb

		if(is_home() && !is_front_page()) {
			$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.optimize_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a>'.$delimiter.' <a '.optimize_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.get_the_title($pageid).'</a></div></div>';

		} elseif(is_home()) {
			$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner">'.$before.$home.$after.'</div></div>';
		} elseif(is_front_page()) {
			if($showOnHome == 1) {
				$output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.optimize_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a></div></div>';
			}
		} else {

			$output .= '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.optimize_mikado_get_inline_style($bread_style).' href="'.$homeLink.'">'.$home.'</a>'.$delimiter;

			if(is_category() || optimize_mikado_is_product_category()) {
				...
				$output .= $before.esc_html__('Search results for ', 'optimize').'"'.get_search_query().'"'.$after;

			} elseif(is_day()) {
				$output .= '<a '.optimize_mikado_get_inline_style($bread_style).' href="'.get_year_link(get_the_time('Y')).'">'.get_the_time('Y').'</a>'.$delimiter;
				$output .= '<a '.optimize_mikado_get_inline_style($bread_style).' href="'.get_month_link(get_the_time('Y'), get_the_time('m')).'">'.get_the_time('F').'</a>'.$delimiter;
				...
				$output .= '<a '.optimize_mikado_get_inline_style($bread_style).' href="'.get_permalink($parent).'">'.$parent->post_title.'</a>';
				if($showCurrent == 1) {
				...
				while($parent_id) {
					...
					$breadcrumbs[] = '<a '.optimize_mikado_get_inline_style($bread_style).' href="'.get_permalink($page->ID).'">'.get_the_title($page->ID).'</a>';
				...
				$output .= $before.esc_html__('Posts tagged ', 'optimize').'"'.single_tag_title('', false).'"'.$after;

			} elseif(is_author()) {
				global $authordata;
				$output .= $before.esc_html__('Articles posted by ', 'optimize').$authordata->display_name.$after;
			...
			if(get_query_var('paged')) {
				...
				$output .= $before." (".esc_html__('Page', 'optimize').' '.get_query_var('paged').")".$after;
...
if(!function_exists('optimize_mikado_get_title_area_height')) {

	/**
	 * Function that returns title height
	 **/
	function optimize_mikado_get_title_area_height() {

		$id = optimize_mikado_get_page_id();
		$title_size = get_post_meta($id, 'mkdf_title_text_size_meta', true);

		$title_height = 97; // default title height without header height

		if(get_post_meta($id, 'mkdf_title_area_subtitle_meta', true) !== '') {
			if($title_size === 'large') {
				$title_height = 135;
			} else {
				$title_height = 110; //default title height when we have subtitle
			}
		}

		if(($meta_temp = get_post_meta($id, "mkdf_title_area_height_meta", true)) != '') {
			$title_height = $meta_temp;
		} elseif(optimize_mikado_options()->getOptionValue('title_area_height') !== '') {
			$title_height = optimize_mikado_options()->getOptionValue('title_area_height');
		}

		return apply_filters('optimize_mikado_title_area_height', $title_height);
	}
}

if(!function_exists('optimize_mikado_get_title_content_padding')) {
	/**
	 * Function that returns title content pading
	 **/

	function optimize_mikado_get_title_content_padding() {
		return apply_filters('optimize_mikado_title_content_padding', 0);
	}
}


if(!function_exists('optimize_mikado_title_area_height')) {
	/**
	 * Function that returns title height and padding to be applied in template
	 **/

	function optimize_mikado_title_area_height() {
		$id                            = optimize_mikado_get_page_id();
		$title_height_and_padding      = array();
		$title_height                  = optimize_mikado_get_title_area_height();
		$header_height_padding         = optimize_mikado_get_title_content_padding();
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_background_image_responsive_meta", true)) != "") {
			...
			$is_img_responsive = optimize_mikado_options()->getOptionValue('title_area_background_image_responsive');
		}

		if(($meta_temp = get_post_meta($id, "mkdf_title_area_vertial_alignment_meta", true)) !== '') {
			$title_vertical_alignment = $meta_temp;
		} else {
			$title_vertical_alignment = optimize_mikado_options()->getOptionValue('title_area_vertial_alignment');;
		}
...
if(!function_exists('optimize_mikado_title_area_background')) {
	/**
	 * Function that returns title background style be applied in template
	 **/

	function optimize_mikado_title_area_background() {
		$id                           = optimize_mikado_get_page_id();
		$show_title_img               = true;
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_background_color_meta", true)) != "") {
			$background_color = $meta_temp;
		} else {
			$background_color = optimize_mikado_options()->getOptionValue('title_area_background_color');
		}
		...
		if(($meta_temp = get_post_meta($id, "mkdf_title_area_background_image_meta", true)) != "") {
			...
			$background_image = optimize_mikado_options()->getOptionValue('title_area_background_image');
		}

		//check for background image width
		$background_image_width = "";
		if($background_image !== '') {
			$background_image_width_dimensions_array = optimize_mikado_get_image_dimensions($background_image);
