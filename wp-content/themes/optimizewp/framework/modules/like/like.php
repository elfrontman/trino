<?php
class OptimizeMikadoLike {

	private static $instance;

	private function __construct() {
		add_action('wp_enqueue_scripts', array( $this, 'enqueue_scripts'));
		add_action('wp_ajax_optimize_mikado_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_optimize_mikado_like', array( $this, 'ajax'));
	}
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'optimize');
		if( isset($_COOKIE['mkdf-like_'. $post->ID]) ){
			$class = 'mkdf-like liked';
			$title = esc_html__('You already like this!', 'optimize');
		}
	...
	public function add_like_portfolio_list($portfolio_project_id){
		...
		return '<a href="#" class="'. $class .'" data-type="portfolio_list" id="mkdf-like-'. $portfolio_project_id .'-'. $rand_number.'" title="'. $title .'"></a>';
	}
...
global $optimize_mikado_like;
$optimize_mikado_like = OptimizeMikadoLike::get_instance();
