...
if(!function_exists('optimize_mikado_single_portfolio')) {
	/**
	 * Loads holder template for portfolio single
	 */
	function optimize_mikado_single_portfolio() {
		$portfolio_template = optimize_mikado_get_portfolio_single_type();
		...
		if($portfolio_template == 'gallery') {
			$params['holder_class'][] = 'mkdf-portfolio-gallery-'.optimize_mikado_options()->getOptionValue('portfolio_single_numb_columns');
		}

		$params['holder_class'] = implode(' ', $params['holder_class']);

		optimize_mikado_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
	}
}

if(!function_exists('optimize_mikado_get_portfolio_single_type')) {
	/**
	 * Returns template of current portfolio item
	 *
	 * @return bool|mixed|void
	 */
	function optimize_mikado_get_portfolio_single_type() {
		return optimize_mikado_get_meta_field_intersect('portfolio_single_template');
	}
}

if(!function_exists('optimize_mikado_get_portfolio_single_media')) {
	/**
	 * Returns a media array of current portfolio item
	 *
	 * @return array
	 */
	function optimize_mikado_get_portfolio_single_media() {
		...
		if(is_array($videos) && count($videos)) {
			usort($videos, 'optimize_mikado_compare_portfolio_videos');
			foreach($videos as $video) {
				...
				if(!empty($video['portfoliovideotype'])) {
					...
					$media['video_url']   = optimize_mikado_portfolio_get_video_url($video);
...
if(!function_exists('optimize_mikado_portfolio_get_video_url')) {
	/**
	 ...
	 * @return array|string
	 */
	function optimize_mikado_portfolio_get_video_url($video) {
...
if(!function_exists('optimize_mikado_portfolio_get_media_html')) {
	/**
	 * Loads template for current portfolio item media
	 *
	 * @param $media
	 */
	function optimize_mikado_portfolio_get_media_html($media) {
		global $wp_filesystem;
		$params = array();

		//For adding image overlay in gallery template type
		$params['gallery'] = (optimize_mikado_get_portfolio_single_type() == 'gallery') ? true : false;

		if($media['type'] == 'image') {
			$params['lightbox'] = optimize_mikado_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
		...
		if(in_array($media['type'], array('youtube', 'vimeo'))) {
			$params['lightbox'] = optimize_mikado_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
		...
		optimize_mikado_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
	}
}

if(!function_exists('optimize_mikado_compare_portfolio_videos')) {
	/**
	 ...
	 */
	function optimize_mikado_compare_portfolio_videos($a, $b) {
...
if(!function_exists('optimize_mikado_compare_portfolio_options')) {
	/**
	 ...
	 * @return int result of comparison
	 */
	function optimize_mikado_compare_portfolio_options($a, $b) {
...
if(!function_exists('optimize_mikado_portfolio_get_info_part')) {
	/**
	 * Loads portfolio info item based on passed param
	 *
	 * @param $part
	 */
	function optimize_mikado_portfolio_get_info_part($part) {
		$portfolio_template = optimize_mikado_get_portfolio_single_type();

		optimize_mikado_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template);
	}
}

if(!function_exists('optimize_mikado_portfolio_get_single_navigation')) {
	/**
	 *
	 */
	function optimize_mikado_portfolio_get_single_navigation() {
		$params = array();

		$in_same_term = optimize_mikado_options()->getOptionValue('portfolio_single_nav_same_category') == 'yes';
		...
		optimize_mikado_get_module_template_part('templates/single/parts/navigation', 'portfolio', '', $params);
	}
}