...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(optimize_mikado_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($row_type == 'row' && $css_animation != ''){
	...
	if($transition_delay !== ''){
		...
		$inner_start .= '<div '.optimize_mikado_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	}else{
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($header_style != ''){
	$wrapper_attributes[] = 'data-mkdf_header_style="'.$header_style.'"';
}
...
if($full_screen_section_height == 'yes'){
	$css_classes[] =  'mkdf-full-screen-height-parallax';
	$after_wrapper_open .= '<div class="mkdf-parallax-content-outer">';
	$before_wrapper_close .= '</div>';

	if($vertically_align_content_in_middle == 'yes'){
		$css_classes[] = 'mkdf-vertical-middle-align';
	}
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="mkdf-mobile-video-image" ' . optimize_mikado_get_inline_attr($video_mobile_style, 'style') . ')"></div>';
	}
	$video_output .= '<div ' . optimize_mikado_get_class_attribute($video_overlay_class) . optimize_mikado_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="mkdf-video-wrap">';
	$video_output .= '<video class="mkdf-video" width="1920" height="800" '. optimize_mikado_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . optimize_mikado_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="Video thumb" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
