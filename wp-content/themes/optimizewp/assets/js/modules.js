...
(function($) {
	...
	common.mkdfSingleImageHover = mkdfSingleImageHover;
...
function mkdfInitVideoBackground(){
	...
	*/
	...
	function mkdfSingleImageHover() {
		var imagesHolder = $('.mkdf-single-image-animation');

		if(imagesHolder.length) {
			imagesHolder.each(function() {
				var image = $(this).find('.vc_single_image-wrapper');

				var timeline = new TimelineLite({ paused: true });

				timeline.to(image, 0.15, { scaleX: 0.95, scaleY: 0.95 });
				timeline.to(image, 0.15, { scaleX: 1, scaleY: 1, ease: Bounce.easeInOut });
...
(function($) {
        ...
        if(progressBar.length){
            ...
            progressBar.each(function() {
                ...
                thisBar.appear(function() {
					...
					TweenLite.to(progressContent, 1.5, { width: percentage + '%' });
    ...
    */
		...
		var processAnimation = function(process) {
			if(!mkdf.body.hasClass('mkdf-no-animations-on-touch')) {
				...
				process.appear(function() {
					...
					tl.staggerFromTo(items, 0.3, {opacity: 0, y: 50, ease: Back.easeOut.config(2)}, {opacity: 1, y: 0, ease: Back.easeOut.config(2)}, 0.2);
				}, {accX: 0, accY: mkdfGlobalVars.vars.mkdfElementAppearAmount});
			}
		}
	...
	function mkdfProgressBarVertical() {
		...
		var animateProgressBar = function(progressBar) {
			...
			progressBar.appear(function() {
				...
				TweenLite.to(activeBar, 1.5, { height: percentage+'%' });
	...
	function mkdfIconWithTextHover() {
		var iconsWithText = $('.mkdf-iwt');

		if(iconsWithText.length) {
			iconsWithText.each(function() {
				var animation = $(this).data('animation');

				if(typeof animation !== 'undefined' && animation) {
					var iconHolder = $(this).find('.mkdf-iwt-icon-holder-inner');
					var timeline = new TimelineLite({ paused: true });

					timeline.to(iconHolder, 0.15, { scaleX: 0.95, scaleY: 0.95 });
					timeline.to(iconHolder, 0.15, { scaleX: 1, scaleY: 1, ease: Bounce.easeInOut });
