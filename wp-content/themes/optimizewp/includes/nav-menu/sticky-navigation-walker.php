 ...
 */
if (!class_exists('OptimizeMikadoStickyNavigationWalker')) {
	class OptimizeMikadoStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			if($depth == 0) {
				$out_div = '<div class="second '.$background_image_class.'" '.optimize_mikado_get_inline_style($menu_image_background).'><div class="inner">';
			} else {
				$out_div = '';
			}
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
			...
			if($item->hide == '') {
				...
				if($item->featured_icon !== ''){
					$featured_icon .= '<span class="mkdf-menu-featured-icon '.$item->featured_icon .'" aria-hidden="true"></span>';
				}

				$item_output .= $featured_icon;

				if(!empty($item->description)) {
					$item_output .= '<span class="mkdf-menu-item-description">'.$item->description.'</span>';
				}
